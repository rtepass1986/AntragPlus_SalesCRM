/**
 * Dashboard Layout
 * Unified layout for all dashboard pages with consistent navigation and branding
 */
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="fixed top-0 left-0 right-0 z-50 bg-white border-b border-gray-200 h-16">
        <div className="flex items-center justify-between h-full px-6">
          ...
          <Link href="/sales" className="flex items-center hover:opacity-80 transition-opacity">
            ...
            />
            <span className="ml-4 text-sm font-medium text-gray-400 border-l border-gray-300 pl-4 hidden sm:inline">
              Sales Cockpit
            </span>
          </Link>
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="relative hidden md:block">
              <input
                type="text"
                placeholder="Suchen..."
                className="w-64 px-4 py-2 pl-10 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-cyan-500"
              />
              <MagnifyingGlassIcon className="absolute left-3 top-2.5 h-5 w-5 text-gray-400" />
            </div>

            {/* Notifications */}
            <button className="relative p-2 text-gray-400 hover:text-gray-600 rounded-lg hover:bg-gray-100" aria-label="Benachrichtigungen">
            ...
            <div className="text-right hidden sm:block">
                ...
                <p className="text-xs text-gray-500">Punkte: {user.score}/10</p>
            </div>
      ...
      {mobileMenuOpen && (
          ...
          <div className="fixed inset-y-0 left-0 w-64 bg-gradient-to-b from-slate-800 to-slate-900">
            <div className="flex items-center justify-between p-4 border-b border-slate-700">
              <span className="text-white font-semibold">Menü</span>
              <button onClick={() => setMobileMenuOpen(false)} className="text-gray-400 hover:text-white" aria-label="Menü schließen">
                <XMarkIcon className="h-6 w-6" />
              </button>
            </div>
            {/* Same navigation as desktop - reuse the nav component */}
            <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
              {navigation.filter(item => item.section === 'main').map((item) => {
                ...
                return (
                  ...
                  >
                    <item.icon className={classNames(isActive ? 'text-cyan-400' : 'text-gray-400 group-hover:text-gray-300', 'mr-3 h-5 w-5 flex-shrink-0')} />
                    {item.name}
                  </Link>
                )
              })}
      ...
      <div className="hidden md:block fixed inset-y-0 left-0 z-40 w-64 bg-gradient-to-b from-slate-800 to-slate-900 pt-16">
        {/* User Info Card */}
        ...
        {/* Navigation */}
        <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto max-h-[calc(100vh-300px)]">
          {/* Main Section */}
          {navigation.filter(item => item.section === 'main').map((item) => {
            const isActive = isCurrentPage(item.href)
            return (
        ...
        {/* Bottom Actions */}
        <div className="absolute bottom-0 left-0 right-0 border-t border-slate-700 p-3 bg-slate-900">
          <button
            onClick={() => {/* logout */}}
      ...
      <div className="md:pl-64 pt-16">
        {/* Breadcrumb */}
        {pathname !== '/sales' && pathname !== '/dashboard' && (
